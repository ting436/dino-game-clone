...
class PlayScene extends GameScene {
    ...
    spawnObstacle() {
        ...
        const obstacleNum = Math.floor(Math.random() * obstaclesCount + 1) + 1;

        const distance = Phaser.Math.Between(100, 350);
        let obstacle;

        if (obstacleNum > PRELOAD_CONFIG.cactusesCount) {
            const enemyPossibleHeight = [20, 70, 100];
            const enemyHeight = enemyPossibleHeight[Math.floor(Math.random() * 3)]
